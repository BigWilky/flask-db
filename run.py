...
def upload():
    user_data1=User.query.filter(id)[-1]
    user_data2=User.query.filter(id)[-2]
    user_data3=User.query.filter(id)[-3]
    ...
    if request.method == 'POST':
        ...
        img = cv2.imread(upload_path,generated_poem)

        cv2.imwrite(os.path.join(basepath, 'static/images/temp', 'test.jpg'), img)
        poem_output=get_poem('static/images/test.jpg')
        user_data=User.query.filter(id)[-1]
        new_id=user_data.id+1;
